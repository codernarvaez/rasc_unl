...
class AuthRemoteRepositoryImpl implements AuthRemoteRepository {
  ...
  @override
  Future<UserApiResponse> updateCurrentUser(String accessToken, Map<String, dynamic> updates) async {
    final url = Uri.parse('$baseUrl/api/v1/auth/me');
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to update current user: ${response.statusCode} - ${response.body}');
    }
  }
  ...
  Future<List<UserApiResponse>> getAllUsers(
    ...
    final url = Uri.parse('$baseUrl/api/v1/auth/users').replace(queryParameters: queryParams);
    final response = await http.get(
      url,
      headers: _getHeaders(accessToken: accessToken),
    );
  ...
  @override
  Future<UserApiResponse> getUserById(String accessToken, int userId) async {
  ...
  @override
  Future<UserApiResponse> updateUser(String accessToken, int userId, Map<String, dynamic> updates) async {
    final url = Uri.parse('$baseUrl/api/v1/auth/users/$userId');
    final response = await http.put(
      url,
      headers: _getHeaders(accessToken: accessToken),
  ...
  @override
  Future<UserApiResponse> activateUser(String accessToken, int userId) async {
  ...
  @override
  Future<UserApiResponse> deactivateUser(String accessToken, int userId) async {
