...
class _ModeratorTimerPageImprovedState extends ConsumerState<ModeratorTimerPageImproved> {
  ...
  Future<void> _saveMarkWithRegistration(TemporaryMark mark) async {
    ...
    if (result == true && mounted) {
      try {
        final provider = ref.read(rascUNLMainProvider);
        
        // Buscar registro por n\u00famero de dorsal
        final dorsalNumber = registrationController.text.trim();
        final allRegistrations = await provider.competitionRegistrationRepository.getAllRegistrations();
        final registration = allRegistrations.firstWhere(
          (r) => r.dorsalNumber == dorsalNumber && r.competenceId == _selectedCompetence!.id,
          orElse: () => throw Exception('Registro con dorsal $dorsalNumber no encontrado'),
        );
        
        await provider.competitionTimeRecordRepository.createTimeRecord(
