...
class _AdminCompetenceDetailsPageState extends ConsumerState<AdminCompetenceDetailsPage> {
  ...
  Future<void> _loadCompetenceDetails() async {
    ...
    try {
      ...
      // Agrupar participantes por número de registro
      ...
      for (var registration in registrations) {
        ...
        if (user != null) {
          final regNumber = registration.registrationNumber ?? 'sin-numero';
          
          if (!groupsMap.containsKey(regNumber)) {
            groupsMap[regNumber] = [];
          }
          
          groupsMap[regNumber]!.add(ParticipantData(
      ...
      final groups = groupsMap.entries.map((entry) {
        return ParticipantGroup(
          registrationNumber: entry.key == 'sin-numero' ? null : entry.key,
          participants: entry.value,
        );
      }).toList();
      
      // Ordenar grupos por número de registro
      groups.sort((a, b) {
  ...
  Future<void> _showEditRegistrationNumberDialog(ParticipantGroup group) async {
    final controller = TextEditingController(text: group.registrationNumber ?? '');
  ...
  List<ParticipantGroup> get _filteredGroups {
    ...
    return _participantGroups.where((group) {
      ...
      if (group.registrationNumber != null &&
          group.registrationNumber!.toLowerCase().contains(_searchQuery.toLowerCase())) {
        return true;
      }
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              '#${group.registrationNumber ?? "Sin asignar"}',
                              style: const TextStyle(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (participant.time != null && participant.time != Duration.zero)
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(
